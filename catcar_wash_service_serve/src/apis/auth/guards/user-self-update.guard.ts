...
import { USER } from '../decorators/roles.decorator';

@Injectable()
export class UserSelfUpdateGuard implements CanActivate {
  private readonly logger = new Logger(UserSelfUpdateGuard.name);
  constructor(private reflector: Reflector) {
    this.logger.log('UserSelfUpdateGuard initialized');
  }
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    this.logger.debug(`Checking user self-update access for ${requestMethod} ${requestPath} with role: ${userRole}`);
    ...
    // Only allow USER role for user self-update
    if (userRole !== USER) {
      throw new PermissionDeniedException('Access denied. This endpoint is only for USER role');
    }

    // Allow access for authenticated users with USER permission
    this.logger.debug('User self-update access granted - user has valid token and USER permission');
    return true;
  }
}
