...
const handleRegisterClick = async () => {
  ...
  // Use Vuetify's built-in validation
  const { valid } = await formRef.value.validate();

  if (!valid) {
    // Form has validation errors - they will be shown by the form fields themselves
...
const registerUser = async () => {
  ...
  try {
    ...
    const selectedPermission = permissionOptions.find(
      (p) => p.value === registrationForm.value.permission_id
    );
